...
public class RationalNumber implements Comparable {
	...
	public int compareTo(Object arg0) {	
		if (!(arg0 == null) 
				&& RationalNumber.class.isAssignableFrom(arg0.getClass())) {
			
			double thisValue = (double) getNumerator() / getDenominator();
			double thatValue = (double) ((RationalNumber) arg0).getNumerator() 
					/ ((RationalNumber) arg0).getDenominator();
		
			if (Math.abs(thisValue - thatValue) < 0.0001)
				return 0;
			else if (thisValue >= (thatValue + 0.0001))
				return 1;
			else
				return -1;
		} else {
